...
with torch.no_grad(): # don't want to compute gradient for all the steps
    ...
    for images, labels1 in test_loader:
        ...
        # value, index
        _, predictions = torch.max(outputs, 1) # will give outputs along with dimension which is 1 which is index. we don't care about values hence we put _ to ignore it
        n_samples += labels1.shape[0] # will give no. of samples in current batch (100)
        n_correct += (predictions == labels1).sum().item() # for each correct prediction we add +1

        class_predictions = [F.softmax(output, dim=0) for output in outputs]
        #The list comprehension iterates over each element in outputs (which is tensor of model outputs).
        # For each element output, it applies the F.softmax function along dim=0.
        # The results are collected into a new list, class_predictions.
    ...
    for i in classes:
        ...
        #Recall answers the question, "Of all the actual positive instances, how many were correctly identified by the model?" 

        #Precision and recall often trade off against each other. In scenarios where you want to minimize false positives, you might prioritize precision, but this could lead to lower recall. Conversely, if you want to minimize false negatives, you might prioritize recall, which could reduce precision.
        writer.close

        # dont confuse ROC/AUC with PR curve
#       PR Curve: Better for understanding the trade-off between precision and recall, especially when false positives are costly.
#       ROC Curve: Provides a broader picture of the model's ability to distinguish between the positive and negative classes.
